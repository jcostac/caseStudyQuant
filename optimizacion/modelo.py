...
def calculate_revenue_metrics(optimizer: BatteryOptimizer, prices_df: pd.DataFrame, window_size: int = 24) -> pd.DataFrame:
    """Calculate revenue metrics (net revenue, costs, cycles etc.) for each time window of prices data.
    
    Args:
        optimizer (BatteryOptimizer): Battery optimizer instance to run simulations
